...
avgfast <- avgfast %>% filter(datatype  %in% c('uint32','uint64','pair'))
# avgfast <- avgfast %>% filter(algo  %in% c("ips2ra","skasort"))
avgfast <- avgfast %>% filter(algo  %in% c("ips2ra","pdqsort"))
