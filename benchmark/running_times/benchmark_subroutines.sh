...
PARALLEL_ALGOS=(benchmark_ips2raparallel benchmark_ips4omlparallel)
...
BUILD_DIR="../../RELEASE_TIMER_${HOSTNAME}"
...
function parallel_subroutines_test {
    ...
    OUTPUT_FILE=b1_parallel_subroutines_${HOSTNAME}_$(mydate)
...
function sequential_subroutines_test {
    ...
    OUTPUT_FILE=b1_sequential_subroutines_${HOSTNAME}_$(mydate)
    print_infos ${OUTPUT_FILE}
    for algo in ${SEQUENTIAL_ALGOS[*]}; do
        ...
        # for type in double uint64; do
        for type in uint64; do
            ${BUILD_DIR}/$algo -b $MIN_LOG_SEQUENTIAL -e $MAX_LOG_SEQUENTIAL -t 1 -g random -v vector -d $type -m ${HOSTNAME} >> ${OUTPUT_FILE}.txt 2>> ${OUTPUT_FILE}.err
            echo "Finished: algorithm=" $algo | tee ${OUTPUT_FILE}.log
        done
    done
}
