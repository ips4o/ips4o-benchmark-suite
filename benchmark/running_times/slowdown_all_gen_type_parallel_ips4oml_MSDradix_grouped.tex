...
%% inner join pfast
%% on ((avgparallel.algo like 'ips4omlparallel'
...
%%  or avgparallel.algo like 'inplacemsdradixsort')
...
%% MIN((case when algo like 'ips4omlparallel' and machine like 'i10pc136' then slowdown else NULL end)) as ips4omlparallela,
%% MIN((case when algo like 'inplacemsdradixsort' and machine like 'i10pc136' then slowdown else NULL end)) as pbbssamplesorta,
%% MIN((case when algo like 'ips4omlparallel' and machine like 'i10pc132' then slowdown else NULL end)) as ips4omlparallelb,
%% MIN((case when algo like 'inplacemsdradixsort' and machine like 'i10pc132' then slowdown else NULL end)) as pbbssamplesortb
%% from slowdown
...
\hline
  ...
  \compiparassssort & \inplacemsdradixsort
  \\\hline
  
  %% TABULAR REFORMAT(col 0-16=(precision=2) row 0-100=(min=bold))
  %% select ips4omlparallelb, pbbssamplesortb from aggrresy83n where datatypename like '\\pair'
  %% and genname similar to '\\distones|\\distsorted|\\distreversesorted'
  \textbf{1.00} & 77.00 \\
  \textbf{1.00} &  7.23 \\
  \textbf{1.00} &       \\
  % END TABULAR select ips4omlparallelb, pbbssamplesortb from aggrresy83n where...
  ...
  %% select
  %% MIN((case when algo like 'ips4omlparallel' and machine like 'i10pc132' then slowdown else NULL end)) as ips4omlparallela,
  %% MIN((case when algo like 'inplacemsdradixsort' and machine like 'i10pc132' then slowdown else NULL end)) as pbbssamplesorta
  %% from slowdownoverall
  \textbf{1.00} & 10.95 \\
  % END TABULAR select MIN((case when algo like 'ips4omlparallel' and machine...))

  %% TABULAR
  %% select
  %% MIN((case when algo like 'ips4omlparallel' and machine like 'i10pc132' then rank else NULL end)) as ips4omlparallela,
  %% MIN((case when algo like 'inplacemsdradixsort' and machine like 'i10pc132' then rank else NULL end)) as pbbssamplesorta
  %% from overallrank
  1 & 2 \\\hline
  % END TABULAR select MIN((case when algo like 'ips4omlparallel' and machine...))
  \end{tabular}
