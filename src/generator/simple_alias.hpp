...
struct simple_alias {
  ...
  // Simple_Alias table member item, used to be a std::tuple<double, size_type, size_type>
  struct tableitem {
  ...
  simple_alias(const simple_alias &other) {
    *this = other;
  }
  ...
  void construct(Iterator begin, Iterator end) {
    ...
    // Assign items
    auto s_begin = work_.data(), s_end = work_.data() + i_small,
      l_begin = s_end, l_end = work_.data() + size_,
...
protected:
  ...
  inline bool is_small(const double& d) const {
    return d <= W_n_;
  }
