...
public:
  ...
  static std::pair<double, double> sort(T* begin, T* end, size_t num_threads) {
    ...
    raduls::RadixSortMSD(reinterpret_cast<uint8_t*>(begin), reinterpret_cast<uint8_t*>(tmp.get()),
                         size, sizeof(T), key_size, num_threads);

    if (key_size % 2 == 1) {
      std_parallel_for(size, [begin, &tmp](size_t start, size_t stop, size_t /* thread*/) {
          std::copy(tmp.get() + start, tmp.get() + stop, begin + start);
        });
      // std::copy(reinterpret_cast<T*>(tmp.get()), reinterpret_cast<T*>(tmp.get()) + size,
      //           begin);
    }
